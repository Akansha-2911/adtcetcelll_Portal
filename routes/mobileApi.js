...
const { documentDir: documentDirectory, pdfDir: pdfDirectory } = require('../utils/storagePaths');
...
router.use((req, res, next) => {
  ...
  const allowAny = process.env.NODE_ENV !== 'production' && allowedOrigins.length === 0;
  const sameOrigin = origin && origin === `${req.protocol}://${req.get('host')}`;
  const allowed = !origin || sameOrigin || allowAny || allowedOrigins.includes(origin);

  if (!allowed) return res.status(403).json({ error: 'Origin is not allowed.' });
  if (origin && (sameOrigin || allowAny || allowedOrigins.includes(origin))) {
...
const mobileLoginLimiter = createRateLimiter({ windowMs: 15 * 60 * 1000, max: 25 });
...
router.post('/student/tests/:testId/start', requireMobileUser, requireRole('student'), async (req, res) => {
  try {
    ...
    if (!result && test.testPassword && String(req.body?.testPassword || '') !== String(test.testPassword)) return res.status(403).json({ error: 'Invalid test password.' });
...
router.get('/student/tests/:testId/questions/:questionNumber', requireMobileUser, requireRole('student'), async (req, res) => {
  try {
    ...
    if (!test) return res.status(404).json({ error: 'Test not found.' });
    const remainingSeconds = test.noTimeLimit ? 86400000 : Math.max(0, Math.floor((test.duration * 60 * 1000 - (Date.now() - new Date(result.startedAt).getTime())) / 1000));
    if (!test.noTimeLimit && !remainingSeconds) return res.status(408).json({ error: 'Test time is over. Submit the test now.' });
...
router.post('/student/tests/:testId/submit', requireMobileUser, requireRole('student'), async (req, res) => {
  try {
    ...
    const markingMap = new Map((test.questionSettings || []).map(row => [String(row.questionId), row]));
    for (const question of test.questions) {
      const marking = markingMap.get(String(question._id));
      const positiveMarks = Number(marking?.positiveMarks ?? question.marks ?? 1);
      const negativeMarks = Number(marking?.negativeMarks ?? test.negativeMarking ?? 0);
      const answerKey = marking?.answerKey || question.correctAnswer;
      const subject = question.subject || 'General';
      ...
      subjectScores[subject].total += positiveMarks;
