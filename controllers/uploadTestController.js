...
const {testUploadDir:UPLOAD_ROOT,toUploadUrl}=require('../utils/storagePaths');
...
function ensureAllowedFile(file){
  if(!file)throw new Error('Required PDF was not uploaded.');
  if(Array.isArray(file))throw new Error('Upload only one PDF in each field.');

  const ext=path.extname(file.name||'').toLowerCase();
  if(!ALLOWED_EXTENSIONS.includes(ext))
    throw new Error('Only PDF files are supported for Upload Test.');

  if(Number(file.size||0)>MAX_FILE_SIZE)
    throw new Error(`Each PDF must be below ${Math.floor(MAX_FILE_SIZE/1024/1024)} MB.`);
}
...
exports.extractTest=async(req,res)=>{
  ...
  try{
    ...
    ensureAllowedFile(questionFile);
    if(solutionFile)ensureAllowedFile(solutionFile);

    const extractionMode='one_question_per_page';
    ...
    const savedQuestionFile=await saveUploadedFile(questionFile,sessionFolder);

    uploadSession.questionFile={
      originalName:savedQuestionFile.originalName,
      fileName:savedQuestionFile.fileName,
      path:savedQuestionFile.path,
      mimeType:savedQuestionFile.mimeType,
      size:savedQuestionFile.size,
      pageCount:0
    };
    ...
    const extraction=await extractUploadedTest({
      questionFilePath:savedQuestionFile.path,
      solutionFilePath:savedSolutionFile?.path||null,
      mode:extractionMode,
      subject,
    ...
    const checkedQuestions=await detectDuplicates(extraction.questions);

    const duplicateCount=checkedQuestions.filter(q=>q.isDuplicate).length;

    uploadSession.questions=checkedQuestions.map((q,index)=>({
      pageNumber:Number(q.pageNumber||index+1),
      solutionPageNumber:q.solutionPageNumber||null,
      ...
      detailedSolution:cleanText(q.detailedSolution),
      ...
      hasVisualQuestion:Boolean(q.hasVisualQuestion),
      hasVisualOptions:Boolean(q.hasVisualOptions),

      fingerprint:q.fingerprint||null,
      isDuplicate:Boolean(q.isDuplicate),
      existingQuestionId:q.existingQuestionId||null,
      duplicateReason:q.duplicateReason||'',

      includeInTest:true,

      extractionStatus:q.extractionStatus||'success',
      extractionMessage:q.extractionMessage||'',

      questionPagePreview:q.questionPagePreview||null,
      solutionPagePreview:q.solutionPagePreview||null
    }));

    uploadSession.totalQuestionPages=
      Number(extraction.questionPageCount)||checkedQuestions.length;

    uploadSession.totalSolutionPages=
      Number(extraction.solutionPageCount)||0;

    uploadSession.questionFile.pageCount=uploadSession.totalQuestionPages;

    if(solutionFile)
      uploadSession.solutionFile.pageCount=uploadSession.totalSolutionPages;

    uploadSession.extractedCount=
      checkedQuestions.filter(q=>q.extractionStatus!=='failed').length;

    uploadSession.failedCount=
      checkedQuestions.filter(q=>q.extractionStatus==='failed').length;

    uploadSession.warningCount=
      checkedQuestions.filter(q=>q.extractionStatus==='warning').length;
    ...
    req.flash(
      'success',
      `${checkedQuestions.length} question(s) extracted. ${duplicateCount} duplicate(s) detected.`
    );

    return res.redirect(
      `/admin/tests/upload-test/review/${uploadSession._id}`
    );
