...
function defaultsFrom(body) {
  const course = COURSES.includes(String(body.course || '').trim())
    ? String(body.course).trim()
    : 'CET';
  const allowedSubjects = SUBJECTS_BY_COURSE[course] || SUBJECTS;
  const requestedSubject = String(body.subject || '').trim();
  return {
    course,
    subject: allowedSubjects.includes(requestedSubject) ? requestedSubject : (allowedSubjects[0] || 'Physics'),
...
exports.commitSmartImport = async (req, res) => {
  ...
  try {
    ...
    selectedQuestions.forEach((question, index) => {
      const missing = ['question','optionA','optionB','optionC','optionD','course','subject']
    ...
    const importAction = 'save_questions';
    const createTest = false;
    ...
    try {
      await session.withTransaction(async () => {
        const questionDocuments = selectedQuestions.map(question => ({
          ...
          course: question.course || importDraft.defaults.course || 'CET',
        ...
        if (createTest) {
          ...
          const validGroups = groupIds.length
            ? await Group.find({ _id: { $in: groupIds }, isActive: true }, '_id').session(session)
            : [];
    ...
    const message = createdTest
      ? `${selectedQuestions.length} questions saved and Draft test created. Configure the marking scheme next.`
      : `${selectedQuestions.length} questions saved to Question Bank.`;
    req.flash('success', message);
    res.redirect(createdTest ? `/admin/tests/${createdTest._id}/marking-template` : '/admin/questions');
  } catch (error) {
