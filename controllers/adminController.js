...
// ── DOWNLOAD TEST PDF / WORD WITH ANSWERS ──────────────────────────────────────────
exports.downloadTestPdfWithAnswers = async (req, res) => {
  try {
    ...
    if (format === 'word') {
      const filename = `Test_${safeFilenamePart(test.title, 'Paper')}_With_Answers.doc`;
...
<div class='meta'><b>${escapeHtml(test.title)}</b> | Subject(s): ${escapeHtml(subjectsStr)} | Duration: ${test.duration || 180} Mins | Total Marks: ${test.totalMarks || 0} | Questions: ${questions.length}</div>
`;
...
<div class='options'>
  <div class='opt ${String(q.correctAnswer).trim().toUpperCase() === 'A' ? 'correct' : ''}'>A) ${formatMathToWordHtml(q.optionA || '')}</div>
  <div class='opt ${String(q.correctAnswer).trim().toUpperCase() === 'B' ? 'correct' : ''}'>B) ${formatMathToWordHtml(q.optionB || '')}</div>
  <div class='opt ${String(q.correctAnswer).trim().toUpperCase() === 'C' ? 'correct' : ''}'>C) ${formatMathToWordHtml(q.optionC || '')}</div>
  <div class='opt ${String(q.correctAnswer).trim().toUpperCase() === 'D' ? 'correct' : ''}'>D) ${formatMathToWordHtml(q.optionD || '')}</div>
</div>
<div class='answer-box'>✔ Correct Answer: Option (${escapeHtml(q.correctAnswer)})
${q.detailedSolution || q.explanation ? `<div class='explanation'><b>Explanation:</b> ${formatMathToWordHtml(q.detailedSolution || q.explanation)}</div>` : ''}
    ...
    const filename = `Test_${safeFilenamePart(test.title, 'Paper')}_With_Answers.pdf`;
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    doc.pipe(res);
    ...
    doc.fontSize(12).font(boldFont).text(test.title, 50, 63, { width: pageW - 20, align: 'center' });
    ...
    questions.forEach((q, idx) => {
      ...
      options.forEach(opt => {
        ...
        const isCorrect = String(q.correctAnswer).trim().toUpperCase() === opt.key;
        const optY = doc.y;
        if (isCorrect) {
...
exports.exportQuestionBank = async (req, res) => {
  try {
    ...
    if (format === 'word') {
      const filename = `SPVN_${subjLabel}_${countLabel}_${timestamp}.doc`;
      let html = `<!DOCTYPE html>
<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'>
...
<div class='meta'>Subject: ${escapeHtml(subject || 'All Subjects')} | Total Questions: ${questions.length} | Generated: ${new Date().toLocaleDateString('en-IN')}</div>
`;
      questions.forEach((q, i) => {
        html += `<div class='question-block'>
...
${q.detailedSolution || q.explanation ? `<div class='explanation'><b>Explanation:</b> ${formatMathToWordHtml(q.detailedSolution || q.explanation)}</div>` : ''}
    ...
    const PDFDocument = require('pdfkit');
    const doc = new PDFDocument({ size: 'A4', margin: 40 });
    const filename = `SPVN_${subjLabel}_${countLabel}_${timestamp}.pdf`;
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    doc.pipe(res);
    ...
    doc.fontSize(9).font(mainFont).fillColor('#94a3b8')
      .text(`Subject: ${subject || 'All Subjects'}   |   Total: ${questions.length} Question(s)   |   Date: ${new Date().toLocaleDateString('en-IN')}`, 50, 68, { width: pageW - 20, align: 'center' });
...
exports.getStudentInfo = async (req, res) => {
  try {
    ...
    const allDivisions = Array.from(new Set(['A', 'B', 'C', 'D', ...distinctDivs.filter(Boolean)])).sort();
...
exports.bulkImportStudentInfo = async (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      const phone = String(row['Phone'] || row['Student Phone'] || row.phone || '').trim() || null;
      const email = String(row['Email'] || row.email || '').trim().toLowerCase() || null;
      const batchName = String(row['Batch'] || row['Group'] || row.batch || '').trim();
      ...
      try {
        ...
        if (student) {
          ...
          const pwd = generatePassword(rollNo);
          student = await User.create({
            name, rollNo, email, phone, parentContact,
            classLevel, division, role: 'student', password: pwd, isFirstLogin: true
          });
          created++;
        }
        ...
        if (batchName) {
          const group = await Group.findOne({ name: new RegExp(`^${batchName}$`, 'i'), isActive: { $ne: false } });
          if (group) {
            await GroupMember.findOneAndUpdate(
              { groupId: group._id, userId: student._id },
              { role: 'student' },
...
exports.downloadStudentInfoTemplate = (req, res) => {
  const rows = [
    { 'Roll No': '2024CE001', 'Name': 'Aarav Sharma', 'Class': '12th', 'Division': 'A', 'Parent Contact No': '9876543201', 'Phone': '9876543211', 'Email': 'aarav@example.com', 'Batch': 'CET Morning Batch' },
    { 'Roll No': '2024CE002', 'Name': 'Ananya Patil', 'Class': '12th', 'Division': 'B', 'Parent Contact No': '9876543202', 'Phone': '9876543212', 'Email': 'ananya@example.com', 'Batch': 'CET Morning Batch' },
    { 'Roll No': '2024CE003', 'Name': 'Rohan Deshmukh', 'Class': '11th', 'Division': 'A', 'Parent Contact No': '9876543203', 'Phone': '9876543213', 'Email': 'rohan@example.com', 'Batch': '' },
  ];
  const wb = xlsx.utils.book_new();
  const ws = xlsx.utils.json_to_sheet(rows);
  ws['!cols'] = [{ wch: 15 }, { wch: 25 }, { wch: 12 }, { wch: 12 }, { wch: 20 }, { wch: 16 }, { wch: 26 }, { wch: 24 }];
...
exports.exportStudentInfoExcel = async (req, res) => {
  try {
    ...
    const data = students.map(s => ({
      ...
      'Class': s.classLevel || '—',
      'Division': s.division || '—',
      'Parent Contact No': s.parentContact || '—',
      'Phone': s.phone || '—',
      'Email': s.email || '—',
    ...
    const wb = xlsx.utils.book_new();
    const ws = xlsx.utils.json_to_sheet(data);
    ws['!cols'] = [{ wch: 15 }, { wch: 25 }, { wch: 12 }, { wch: 12 }, { wch: 20 }, { wch: 16 }, { wch: 26 }, { wch: 24 }, { wch: 14 }];
    xlsx.utils.book_append_sheet(wb, ws, 'Students');
    const buf = xlsx.write(wb, { type: 'buffer', bookType: 'xlsx' });
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
